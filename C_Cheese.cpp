...
int main()
{
    ...
    for (int i = 0; i < n; ++i)
    {
        if (k > vp[i].first)
        {
            answer += vp[i].second * (1LL) * (vp[i].first);
            k -= vp[i].first;
        }
        else
        {
            answer += vp[i].second * (1LL) * (k);
            break;
        }
    }
    long long count = 0;
    ...
    for (int i = 0; i < n; ++i)
    {
        if (k > vp1[i].second)
        {
            count += vp1[i].first * (1LL) * (vp1[i].second);
            k -= vp1[i].second;
        }
        else
        {
            count += vp1[i].first * (1LL) * (k);
            break;
        }
    }
    cout << max(answer,count) << "\n";
}