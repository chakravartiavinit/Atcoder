...
int main()
{
    ...
    debug(ans);
    debug(ans2);
    if (ans.size() < 2 && ans2.size() < 1)
    {
        cout << "0\n";
    }
    else
    {
        long long finalans = (ans[ans.size() - 1] * (1LL) * (ans[ans.size() - 2]));
        debug(finalans);
        if (ans2.size() > 0)
        {
            long long finalans2;
            finalans2 = (ans2[ans2.size() - 1]) * (1LL) * (ans2[ans2.size() - 1]);
            cout << max(finalans2, finalans) << "\n";
        }
        else
        {
            cout << finalans << "\n";
