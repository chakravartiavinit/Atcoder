...
void printAllKLengthRec(char set[], string prefix,
                        int n, int k, vector<string> &beta)
{
    if (k == 0)
    {
        beta.push_back(prefix);
        return;
    }
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        printAllKLengthRec(set, newPrefix, n, k - 1, beta);
    }
}
int main()
{
    ...
    for (int i = 0; i < 4; ++i)
    {
        printAllKLengthRec(set, "", n, i, beta);
    }
    int t;
    cin >> t;
    while (t--)
    {
        ...
        {
            string temp = beta[i];
            if (alpha.find(temp) == string::npos)
            {
                cout << temp << "\n";
                break;
            }
