...
int main()
{
    ...
    for (int i = 0; i < n; ++i)
    {
        if (i == 0)
        {
            long long temp = abs(arr[0] - arr[n - 1]);
            long long temp2 = abs(arr[0] - arr[1]);
            vp.push_back({min(temp, temp2), max(temp, temp2)});
        }
        else if (i == n - 1)
        {
            vp.push_back({min(abs(arr[n - 1] - arr[0]), abs(arr[n - 1] - arr[n - 2])), max(abs(arr[n - 1] - arr[0]), abs(arr[n - 1] - arr[n - 2]))});
        }
        else
        {
            long long temp = abs(arr[i] - arr[i - 1]);
            long long temp3 = abs(arr[i] - arr[0]);
            long long temp2 = abs(arr[i] - arr[n - 1]);
            long long temp4 = abs(arr[i] - arr[i + 1]);
            vp.push_back({min(temp, temp4), max(temp3, temp2)});
        }
    }
    for (auto x : vp)
    {
